...
describe("usersScoreApi - requestMatchScore", () => {
  ...
  it("envía payload correcto para partidas 1vs1 y parsea score", async () => {
    ...
    const mockResponse = {
      ok: true,
      json: vi.fn().mockResolvedValue({ score: 350, saved: true, gameId: 21 }),
    };
    ...
    expect(result).toEqual({ score: 350, saved: true, gameId: 21 });
  });
  ...
  it("envía payload correcto para partidas 1vsbot y parsea score desde points", async () => {
    ...
    const mockResponse = {
      ok: true,
      json: vi.fn().mockResolvedValue({ points: "420", saved: true, gameId: 22 }), // string
    };
    ...
    expect(result).toEqual({ score: 420, saved: true, gameId: 22 }); // convertido a número
  });
  ...
  it("también acepta puntuacion/puntuacion en la respuesta", async () => {
    ...
    const mockResponse = {
      ok: true,
      json: vi.fn().mockResolvedValue({ puntuacion: "99", saved: true, gameId: 23 }),
    };

    global.fetch.mockResolvedValue(mockResponse);

    const result = await requestMatchScore(matchSummary);
    expect(result).toEqual({ score: 99, saved: true, gameId: 23 });
  });
